...
namespace Project.BussinessRules
{
    public class CatalogoCliente
    {
        ...
        public void addLugar(LugarTratamiento ac)
        {
            try
            {
                DataAccess.DataBase bd = new DataAccess.DataBase();
                bd.connect();
                string sql = "insert into lugartratamiento values('"+ac.CODNT+"','"+ ac.NombreTrata +"','"+ac.Ruta+"','"+ac.COD+"','"+ ac.RUT +"','"+ac.TIPO+"');";
                bd.CreateCommand(sql);
                bd.execute();
                bd.close();
            }
     ...
     * */
        public List<cliente> getmaxcodcli()
        {
            ...
            bd.connect();
            List<cliente> units = new List<cliente>();
        //    string sql = "select last_insert_id(max(Cod)+1) from cliente ;";  
         //   string sql = "select last_insert_id(ifnull(max(Cod),0+1)) from cliente ";  
        ...
        public List<cliente> mostrarplanilla(string s)
        {
            try
            {
                ...
                //    string sql = "select usuario.RUT_US,usuario.NOM_US,usuario.APELL_US,usuario.CORR_US,usuario.USER_US,usuario.PASS_US,usuario.NAC_US,usuario.ROL_US,alumno.ID_SAL,alumno.CUR_ALUM from usuario,alumno where usuario.RUT_US like alumno.RUT_US";
                //   string sql = "select tecnicos.Nombre,servicio.Nombre,cliente.Nombre,consumo,area from tecnicos,servicio,cliente inner join planilla where planilla.Tecnicos_Cod=tecnicos.Cod and planilla.Servicio_Cod=servicio.Cod and planilla.Cliente_Rut=cliente.Rut";
              //  string sql = "select cliente.cod, cliente.Nombre, cliente.Rut, cliente.Clasificacion, lugartratamiento.NombreTratamiento from cliente, lugartratamiento where lugartratamiento.cliente_Cod = cliente.Cod;";
        ...
        public List<Inventariadoplano> mbuscarlugarplano(string s)
        {
            try
            {
                ...
                //    string sql = "select usuario.RUT_US,usuario.NOM_US,usuario.APELL_US,usuario.CORR_US,usuario.USER_US,usuario.PASS_US,usuario.NAC_US,usuario.ROL_US,alumno.ID_SAL,alumno.CUR_ALUM from usuario,alumno where usuario.RUT_US like alumno.RUT_US";
                //   string sql = "select tecnicos.Nombre,servicio.Nombre,cliente.Nombre,consumo,area from tecnicos,servicio,cliente inner join planilla where planilla.Tecnicos_Cod=tecnicos.Cod and planilla.Servicio_Cod=servicio.Cod and planilla.Cliente_Rut=cliente.Rut";
                //  string sql = "select cliente.Nombre,cliente.Rut,cliente.Clasificacion,lugartratamiento.NombreTratamiento from cliente,lugartratamiento where  and lugartratamiento.NombreTratamiento like '%" + s + "%'";
                //string sql = "select cliente.cod, cliente.Nombre, cliente.Rut, cliente.Clasificacion, lugartratamiento.NombreTratamiento from cliente, lugartratamiento where lugartratamiento.cliente_Cod = cliente.Cod and cliente.nombre like '%"+s+"%'";
                string sql = "select cliente.cod,lugartratamiento.NombreTratamiento,cliente.Rut,plano.Codplano, lugartratamiento.RUTA from cliente,plano, lugartratamiento where lugartratamiento.cliente_Rut = cliente.Rut and plano.lugartratamiento_Cod=lugartratamiento.Cod and cliente.nombre like '%" + s + "%'";
                db.CreateCommand(sql);
                ...
                while (result.Read())
                {
                   ...
                   Inventariadoplano a = new Inventariadoplano(codcod, codnom, codrut, codclasi,codruta);
                    tec.Add(a);
                }
        ...
        public List<InventariadoCliente> mbuscarcontacto(string s)
        {
            try
            {
                ...
                //  string sql = "select cliente.Nombre,cliente.Rut,cliente.Clasificacion,lugartratamiento.NombreTratamiento from cliente,lugartratamiento where  and lugartratamiento.NombreTratamiento like '%" + s + "%'";
                //string sql = "select cliente.cod, cliente.Nombre, cliente.Rut, cliente.Clasificacion, lugartratamiento.NombreTratamiento from cliente, lugartratamiento where lugartratamiento.cliente_Cod = cliente.Cod and cliente.nombre like '%"+s+"%'";
                string sql = "  select cliente.Cod, lugartratamiento.RUTA, lugartratamiento.NombreTratamiento, contacto.NombreContacto, contacto.Email,contacto.Telefono,contacto.TipoContacto,plano.Codplano, contacto.id, cliente.rut, lugartratamiento.cod, cliente.nombre from lugartratamiento, contacto, plano,cliente where plano.lugartratamiento_Cod=lugartratamiento.Cod and lugartratamiento.cliente_Cod=cliente.Cod and contacto.lugartratamiento_Cod= lugartratamiento.Cod and lugartratamiento.cliente_Rut= cliente.Rut  and cliente.Nombre like'%" + s + "%'";
                db.CreateCommand(sql);
                ...
                while (result.Read())
                {
                    ...
                    InventariadoCliente a = new InventariadoCliente(codcod, codruta, codlugar, codnombre, codcorreo,codtelefono,codtipo,codplano,codid, codrut, codcli, codnombrecliente);
                    tec.Add(a);
                }
